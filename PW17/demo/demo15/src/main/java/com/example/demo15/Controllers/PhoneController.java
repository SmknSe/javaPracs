...
public class PhoneController {
    ...
    @GetMapping(value = "/sort_phones")
    public ResponseEntity<List<Phone>> getSortedPhones() {
        final List<Phone> phones = serviceTableService.sortPhonesByName();
        return phones != null && !phones.isEmpty()
                ? new ResponseEntity<>(phones, HttpStatus.OK)
                : new ResponseEntity<>(HttpStatus.NOT_FOUND);
    }

    @GetMapping(value="/phones/filter/{name}")
    public ResponseEntity<List<Phone>> getFilteredPhones(@PathVariable(name="name") String name) {
        System.out.println(name);
        final List<Phone> phones = serviceTableService.filterPhonesByName(name);
        return phones != null && !phones.isEmpty()
                ? new ResponseEntity<>(phones, HttpStatus.OK)
                : new ResponseEntity<>(HttpStatus.NOT_FOUND);
    }
